...
// Returns `Result` to mirror `get_amount_0_delta`'s signature (which can fail on a zero price);
// this variant never errors but stays fallible so both deltas compose uniformly.
...
mod tests {
    ...
    fn test_get_sqrt_ratio_at_tick_is_valid_min_tick() {
        assert_eq!(get_sqrt_ratio_at_tick(MIN_TICK).unwrap(), BigInt::from(4_295_128_739_u128));
    }

    #[test]
    fn test_get_sqrt_ratio_at_tick_is_valid_min_tick_add_one() {
        assert_eq!(get_sqrt_ratio_at_tick(MIN_TICK + 1).unwrap(), BigInt::from(4_295_343_490_u128));
    }
    ...
    fn test_get_sqrt_ratio_at_tick_is_less_than_js_impl_min_tick() {
        let js_min_sqrt_price = BigInt::from(6_085_630_636u64);
    ...
    fn test_calculate_token_amounts() {
        let cases: [(BigInt, i32, i32, i128, BigInt, BigInt); 2] = [
            (
                BigInt::from(79_228_162_514_264_337_593_543_950_336_i128),
                -887_270,
                887_270,
                2_779_504_125,
                BigInt::from(2_779_504_125_u128),
                BigInt::from(2_779_504_125_u128),
            ),
            (
                BigInt::from(17_189_630_842_187_678_489_986_852_982_138_i128),
                -138_200,
                107_600,
                -79_381_057_257_465_377_800_177,
                BigInt::from(-445_577_797_388_351_i128),
                BigInt::from(-17_222_724_212_376_326_765_220_041_i128),
            ),
        ];

        for (current_sqrtprice, tick_lower, tick_upper, liquidity_delta, exp0, exp1) in cases {
            let (amount0, amount1) =
                calculate_token_amounts(current_sqrtprice, tick_lower, tick_upper, liquidity_delta)
                    .unwrap();

            assert_eq!(amount0, exp0);
            assert_eq!(amount1, exp1);
        }
    }
}
